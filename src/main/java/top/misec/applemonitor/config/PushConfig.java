...
public class PushConfig {
    ...
    public PushInfo getPushInfo() {
        if (StrUtil.isAllNotBlank(TG_BOT_TOKEN, TG_USER_ID) && Boolean.TRUE.equals(TG_USE_CUSTOM_URL)) {
            return new PushInfo(new TelegramCustomUrlPush(), TG_BOT_TOKEN, TG_USER_ID);
        } else if (StrUtil.isAllNotBlank(TG_BOT_TOKEN, TG_USER_ID)) {
            return new PushInfo(new TelegramPush(), TG_BOT_TOKEN, TG_USER_ID);
        } else if (StrUtil.isAllNotBlank(DING_TALK_URL, DING_TALK_SECRET)) {
            return new PushInfo(new DingTalkSecretPush(), DING_TALK_URL, null, DING_TALK_SECRET);
        } else if (StrUtil.isNotBlank(DING_TALK_URL)) {
            return new PushInfo(new DingTalkPush(), DING_TALK_URL);
        } else if (StrUtil.isNotBlank(PUSH_PLUS_TOKEN)) {
            return new PushInfo(new PushPlusPush(), PUSH_PLUS_TOKEN);
        } else if (StrUtil.isNotBlank(SCT_KEY)) {
            return new PushInfo(new ServerChanTurboPush(), SCT_KEY);
        } else if (StrUtil.isNotBlank(WE_COM_TOKEN)) {
            return new PushInfo(new WeComPush(), WE_COM_TOKEN);
        } else if (StrUtil.isNotBlank(SC_KEY)) {
            return new PushInfo(new ServerChanPush(), SC_KEY);
        } else if (StrUtil.isAllNotBlank(WE_COM_APP_CORP_SECRET, WE_COM_APP_CORPID) && null != WE_COM_APP_AGENT_ID) {
...
//        }
//
//        if (StrUtil.isNotBlank(PROXY_HTTP_HOST)) {
//            return new HttpHost(PROXY_HTTP_HOST, PROXY_PORT, "http");
//        }
//
//        if (StrUtil.isNotBlank(PROXY_SOCKET_HOST)) {
